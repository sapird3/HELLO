...
def process_excel(data_file, new_file_name, perc_est, mapping_file):
    ...
    data = []
    feature_teams = set()
    feature_key_list = []  # AC: confirm to match sure there are no repeated keys
    ...
    for row in range(2, data_last_row + 1):  # Start from 2 to skip the header row
        ...
        art = ws1.range('K' + str(row)).value  # art_scrum_team
        ...
        if issue_type == 'Feature':
            feature_teams = set()
            feature_key = key
            feature_key_list.append(feature_key)  # dont need?
            feature_unique_id_list.append(unique_id)  # dont need?
            feature_children[feature_key] = set()  # dont need?
            feature_ref_level = refinement_lvl

            # AC: these are not needed 
...
'''
        if art is not None: # if issue type = everything?
            if mapping_file != '':
