...
def process_excel(data_file, new_file_name, perc_est, mapping_file):
    ...
    for row in range(2, data_last_row + 1):  # Start from 2 to skip the header row
        ...
        if issue_type != 'Feature' and issue_type != 'Issue' and issue_type != 'User Story':  # AC: Should this also include "Feature Estimation"? I'm also wondering if the code could be restructured to have one section only for Features and another sections for Stories, Issues and Feature Estimation tickets
            continue  # Skip rows that are not Features, Issues, or User Stories (or those that are empty) 
        ...
        if art is not None:
            ...
            if resource_names not in feature_teams:
                feature_teams.add(resource_names)  # AC: Is this compiling the list of teams working on specific feature? If so, should we also include "Feature Estimations" to the if statement on line 102
                #print(feature_teams)
            else:
        ...
        focus_duration = '1 hr' # AC: Could we put lines 177 and 178 under the If statement in 180 since this only should apply to features?
        ...
        if issue_type == 'Feature':
            ...
            if refinement_lvl == 'Feature Estimation': # AC: How can we assign Feature estimation values at this point, when we haven't pulled in data from Feature Estimation tickets yet? (i.e. it's not part of the IF statment on line 102)
