...
def read_excel_file(file_path, sheet_num):
    """
...
def process_data(ws1, ws2, wb3, milestone_map, issue_type_data, status_data, map_file_last_row, ms_map_file_last_row, perc_est): #AC: Removed ws3 after ws2
    """
    ...
    """
    data = []
    feature_key_dict = {}  # Use a dictionary to store keys and their line numbers
    key_to_unique_id = {}
    unique_id_counter = 1
    feature_status = 'Open'
    ...
    for row in range(2, len(issue_type_data) + 1):
        ...
        if key in feature_key_dict:
            previous_row = feature_key_dict[key]
            print(f'This key was found again: {key}, previously found at row: {previous_row}')
            populate_log(wb3, 'Repeated Key', f'Previous Row: {previous_row}, Current: {row}', key)
            continue
        else:
            feature_key_dict[key] = row  # Store the current row number for this key
        ...
        if issue_type == 'Feature':
            ...
            feature_status = status
            milestone = ws1.range(f'Q{row}').value
            
            milestone_dict = milestone_conversion(milestone_map, ms_map_file_last_row)
            print(f'SUCC LIST: {succ_list}')
            for item in succ_list:
                if milestone in milestone_dict:
                    milestone_key = milestone_dict[milestone]
                    print(f'MILESTONE: {milestone}')
                    print(f'MILESTONE KEY!!!! {milestone_key}')
                    print(f'ITEM IN SUCC LIST: {item}')
                    if item in milestone_dict:
                        if item == milestone_key or item == milestone:
                            print('DUPLICATE MILESTONE KEY TO SUCCESSOR: LOG ERROR')
                            populate_log(wb3, 'Duplicate Milestone (Key already in Successor List)', item, selector)
                        else:
                            print('MATCHES WRONG MILESTONE')
                            populate_log(wb3, 'Milestone does not match successor link', item, selector)
                    else:
                        # Check if 'milestone' should be added to 'succ_list' based on a condition
                        succ_list.append(milestone)
                elif milestone is not None:
                    print(f"Milestone '{milestone}' not found in the dictionary.")
                    populate_log(wb3, 'Milestone not found in the dictionary', milestone, selector)
        ...
        if resource_names not in feature_teams:         #AC: Added based on comments in 156
            ...
            selector, task_name, focus_duration, low_risk_duration, perc_complete, task_type = get_task_details(
                issue_type, feature_key, resource_names, feature_summary, status, feature_ref_level, sum_total_sp, sum_done_sp, realistic_est, worst_case_est, perc_est, feature_key, wb3) #AC: compared row201, this was missing refinement level I think this should be Feature Refinement Level. Updated.
            ...
            if child_index == -1:
                ...
                populate_log(wb3, 'Resource Name Not Found', resource_names, item)
                continue
            ...
            if feature_ref_level == "Story Points":
                ...
                if add_total_sp is not None:
                    add_total_sp = float(add_total_sp)
                else:
                    add_total_sp = 0.0
                    populate_log(wb3, 'Story Point with 0 Effort', 'Missing Sum Total SP', selector)
                ...
                if add_done_sp is not None:
                    add_done_sp = float(add_done_sp)
                else:
                    add_done_sp = 0.0
                    populate_log(wb3, 'Story Point with 0 Effort', 'Missing Sum Done SP', selector)
                ...
                if new_total_sp is not None:
                    data[child_index]['remaining_sp'] = new_total_sp - new_done_sp
                else:
                    data[child_index]['remaining_sp'] = 0
                    populate_log(wb3, 'Story Point with 0 Effort', 'Missing Sum SP', selector)

            if feature_ref_level == "Feature Estimation":
                # realistic est
                if data[child_index]['realistic_est'] is not None:
                    old_real_est = float(data[child_index]['realistic_est'])
                else:
                    old_real_est = 0.0
                    populate_log(wb3, 'Feature Estimation with 0 Effort', 'Missing Realistic Estimate', selector)
                add_real_est = ws1.range('M' + str(row)).value
                if add_real_est is not None:
                    add_real_est = float(add_real_est) 
                else: 
                    add_real_est = 0.0
                    populate_log(wb3, 'Feature Estimation with 0 Effort', 'Missing Realistic Estimate', selector)
                ...
                # worst case est
                if data[child_index]['worst_case_est'] is not None:
                    old_wc_est = float(data[child_index]['worst_case_est'])
                else:
                    old_wc_est = 0.0
                    populate_log(wb3, 'Feature Estimation with 0 Effort', 'Missing Worst Case Estimate', selector)
                add_wc_est = ws1.range('N' + str(row)).value
                if add_wc_est is not None:
                    add_wc_est = float(add_wc_est)
                else:
                    add_wc_est = 0.0
                    populate_log(wb3, 'Feature Estimation with 0 Effort', 'Missing Worst Case Estimate', selector)
    ...
    #update_unique_ids(data, wb3, key_to_unique_id)

    print ("Collecting data complete")
    
    return data, key_to_unique_id
    

def get_task_details(issue_type, key, resource_names, summary, status, refinement_lvl, sum_total_sp, sum_done_sp, realistic_est, worst_case_est, perc_est, feature_key, wb3):
    """
    ...
    if issue_type == 'Feature':
        ...
        if refinement_lvl == 'Feature Estimation':
            ...
            populate_log(wb3, 'Missing Refinement Level', id, feature_key)
    ...
    if task_name.endswith('\t'):  # DS added 10/29
        task_name = task_name[:-1]

    return selector, task_name, focus_duration, low_risk_duration, perc_complete, task_type

def milestone_conversion(milestone_map, ms_map_file_last_row):

    milestone_dict = {}

    for row in range(2, ms_map_file_last_row + 1):
        milestone = milestone_map.range('A' + str(row)).value
        key = milestone_map.range('B' + str(row)).value
        milestone_dict[milestone] = key
        
    print(f'Milestone Dictionary created: {milestone_dict}')
...
def update_unique_ids(data, ws3, key_to_unique_id):
    ...
    """
    for item in data:
        unique_id_succ = get_unique_ids(item['succ_list'], key_to_unique_id)
        item['unique_id_succ'] = unique_id_succ

        unique_id_pred = get_unique_ids(item['pred_list'], key_to_unique_id)
        item['unique_id_pred'] = unique_id_pred

def get_unique_ids(id_list, key_to_unique_id):
    """
...
def create_excel_file(file_path):          
    ...
    try:
        ...
        wb.sheets[0].range('A1').value = 'Please be patient as the script runs and loads the data.'
        return wb
    except Exception as e:
        print(f"An error occurred while creating the Excel file: {e}")
        populate_log(wb, 'Error Creating Excel File', e)
    finally:
        print("did not use wb.close()")             #AC change back to wb.close() once file location is working.

def create_sheets_in_excel(wb3, sheet_data):
    """
    ...
    try:
        ...
        populate_log(wb3, 'Error Creating Sheets', e)

def load_data (data, wb3, key_to_unique_id, milestone_map, ms_map_file_last_row):
    """
    ...
    for item in data:
        ...
        ws3.range('U' + str(proj_row)).value = item['milestone']
...
def map_uid(data, wb3):
    ...
    for item in data:
        ...
        for id in item['succ_list']:
            if id in key_to_unique_id:
                ...
                populate_log(wb3, 'Missing Unique ID Successor', id, item['key'])
        ...
        for id in item['pred_list']:
            if id in key_to_unique_id:
                ...
                print ("Dependency ID not found in input file: ",id)
                populate_log(wb3, 'Missing Unique ID Predecessor', id, item['key'])
    ...
    print('Mapping of UIDs completed with logs for missing IDs.')
...
def resize_columns(wb):
    ...
    '''
    for ii in range (3):
        wb.sheets[ii].autofit('c')
        wb.sheets[ii].autofit('r')

def populate_summary(wb3, data):
    """
    Populate the Summary sheet with unique resource names and their accumulated focus durations.
    
    Args:
        ws (xlwings.Sheet): The Summary worksheet object.
        data (list): The list of processed data.
    """
    ws3 = wb3.sheets['Summary']

    summary_dict = {}
    total_sum = 0
    
    # Aggregate focus duration for each resource
    for item in data:
        resource_name = item['resource_names']
        focus_duration = item['focus_duration']
        
        if resource_name is not None:
            if resource_name not in summary_dict:
                    summary_dict[resource_name] = focus_duration
                    total_sum += focus_duration
            else:
                if focus_duration == '1 hr':
                    summary_dict[resource_name] += 0
                else:
                    summary_dict[resource_name] += focus_duration
                    total_sum += focus_duration
    
    row = 2
    for resource, total_duration in summary_dict.items():
        ws3.range(f'A{row}').value = resource
        ws3.range(f'B{row}').value = total_duration
        row += 1
    
    ws3.range(f'A{row}').value = 'Total FDR:'
    ws3.range(f'B{row}').value = total_sum

    print('Summary sheet populated.')

def populate_log(wb, event, error_data, selector):
    '''
    Log any error occurances.
    '''
    
    ws_log = wb.sheets['Log']

    log_row = ws_log.range('A' + str(ws_log.cells.last_cell.row)).end('up').row
    
    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    ws_log.range(f'A{log_row + 1}').value = timestamp
    ws_log.range(f'B{log_row + 1}').value = selector
    ws_log.range(f'C{log_row + 1}').value = event
    ws_log.range(f'D{log_row + 1}').value = error_data

def finalize_log(wb3, data_file_path, mapping_file_path, duration_sec, duration_min):
    '''
    Add important info to the bottom of the Log (file paths, important dates, script execution durations)
    '''
    ws = wb3.sheets['Log']
    last_row = ws.range('A' + str(ws.cells.last_cell.row)).end('up').row

    ws.range(f'A{last_row + 2}').value = 'Input Data File Path:'
    ws.range(f'B{last_row + 2}').value = data_file_path
    
    ws.range(f'A{last_row + 3}').value = 'Input Mapping File Path:'
    ws.range(f'B{last_row + 3}').value = mapping_file_path

    ws.range(f'A{last_row + 4}').value = 'Creation Date:'
    ws.range(f'B{last_row + 4}').value = f"'{datetime.now().strftime('%m/%d/%Y')}"

    ws.range(f'A{last_row + 5}').value = 'Latest Update:'
    ws.range(f'B{last_row + 5}').value = f"'{datetime.now().strftime('%m/%d/%Y')}"

    ws.range(f'A{last_row + 7}').value = 'Script Execution Time (sec):'
    ws.range(f'B{last_row + 7}').value = f'{duration_sec:.2f} seconds'

    ws.range(f'A{last_row + 8}').value = 'Script Execution Time (min):'
    ws.range(f'B{last_row + 8}').value = f'{duration_min:.2f} minutes'
