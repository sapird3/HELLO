...
# Version 5) Converted to a defined function - repeats teams under feature

# import program to grab data from excel
import xlwings as xw

def process_DANS(data_file, mapping_file, new_file_name):
    ...
    for row in range(2, data_last_row + 1):  # Start from 2 to skip the header row
        ...
        # NEWEST FEATURE: MAPPING TEAM RENAMING
        for map_row in range(2, map_last_row + 1):
            if art == ws3.range('A' + str(map_row)).value:
                art = ws3.range('B' + str(map_row)).value

                # Check if the team already exists in this feature
                if art not in feature_teams:
                    feature_teams.add(art)
                else:
...
if __name__ == "__main__":
    ...
    process_DANS(data_file, mapping_file, new_file_name)
