...
# Version 3) Compacting team rows into feature key "_" team name (as well as feature estimations)
...
# NEWEST input: IRR_Medium_Term_NEW_240718_1045.xlsx
...
file = input('Enter the name of the file you would like to input into this program: ') + '.xls' 
...
for value in range(2, last_row + 1):  # Start from 2 to skip the header row
    ...
    if issue_type is None: # clears empty rows **BUT HAVE DATA AT THE END**
        continue
    ...
    # empty team names for new feature
    if issue_type == 'Feature':
        feature_teams = set()
        feature_key = key
 
    sum_total_sp = ws1.range('N' + str(value)).value
    sum_groom_sp = ws1.range('O' + str(value)).value
        ...
        if len(part2) >= 2:
            ...
            if resource_names not in feature_teams:
                ...
                if sum_total_sp is not None:
                    sum_total_sp += ws1.range('N' + str(value)).value
                if sum_done_sp is not None:
                    sum_done_sp += ws1.range('O' + str(value)).value
                if realistic_est is not None:
                    realistic_est += ws1.range('L' + str(value)).value
                if worst_case_est is not None:
                    worst_case_est += ws1.range('M' + str(value)).value
                continue
